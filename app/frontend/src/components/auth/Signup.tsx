...
const Signup: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check password complexity (backend will also validate)
    ...
    try {
      ...
      if (data.user) {
        // For signup, we need to login separately to get the token
        ...
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl shadow-slate-200/50 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-2 py-3 px-4 rounded-xl bg-gradient-to-r from-indigo-600 via-blue-600 pink-500 text-white font-semibold text-sm shadow-lg shadow-blue-500/30 hover:shadow-blue-500/40 hover:opacity-95 disabled:opacity-70 disabled:cursor-not-allowed transition-all duration-200"
            >
